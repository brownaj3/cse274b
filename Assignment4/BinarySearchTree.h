...
protected:
    ...
    virtual void getLE(Node *u, DLList<T> &list, T z);
public:
...
void BinarySearchTree<Node, T>::printAll(Node *u) {
    ...
    std::cout << "Preorder: " << u->preorderNumber << std::endl;
    std::cout << "Postorder: " << u->postorderNumber << std::endl;
    std::cout << "Inorder: " << u->inorderNumber << std::endl;
...
template<class Node, class T>
void BinarySearchTree<Node, T>::getLE(Node* u, DLList<T> &list, T z) {
    if (u == nil) {
        return;
    }
    else if (u->x < z) {
